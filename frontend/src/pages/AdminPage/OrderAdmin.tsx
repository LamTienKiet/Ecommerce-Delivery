...
export const OrderAdmin = () => {
  ...
  const handleUpdateStatus = async (orderId: number, newStatus: string) => {
    try {
      ...
      setOrders(
        orders.map((o) =>
          o.id === orderId ? { ...o, currentStatus: newStatus } : o,
        ),
      );
    } catch (error) {
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Chờ Xử Lý",
            value: orders.filter((o) => o.currentStatus === "PENDING").length,
            color: "text-amber-600",
            bg: "bg-amber-50",
          },
          {
            label: "Đang Chuẩn Bị",
            value: orders.filter((o) => o.currentStatus === "PREPARING").length,
            color: "text-blue-600",
            bg: "bg-blue-50",
          },
          {
            label: "Hoàn Thành",
            value: orders.filter((o) => o.currentStatus === "COMPLETED").length,
                    ...
                    <p>
                      ...
                      <span className="font-bold text-slate-800">
                        {selectedOrder.fullName ||
                          (selectedOrder as any).user?.fullName}
                      </span>
                    </p>
                    <p>
                      ...
                      <span className="font-medium text-slate-800">
                        {selectedOrder.phone ||
                          (selectedOrder as any).user?.phone}
                      </span>
                    </p>
                    <p>
                      <span className="text-slate-500 font-medium">
                    ...
                    <p>
                      ...
                      <span className="font-bold text-indigo-600">
                        {selectedOrder.paymentMethod ||
                          (selectedOrder as any).payment?.paymentMethod ||
                          "N/A"}
                      </span>
                    </p>
                    <p>
                      ...
                      <span
                        className={`ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${
                          STATUS_OPTIONS.find(
                            (o) => o.value === selectedOrder.currentStatus,
                          )?.color
                        }`}
                      >
                        {
                    ...
                    {selectedOrder.orderItems?.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50">
                        <td className="px-4 py-3">
                          ...
                          {item.note && (
                            <div className="text-xs text-amber-600 mt-0.5 flex items-start gap-1">
                              <svg
