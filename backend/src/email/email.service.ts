...
export class EmailService {
  ...
  async sendOrderConfirmation(email: string, orderId: number, totalAmount: number) {
    ...
    try {
      ...
      this.logger.log(`Email xác nhận đơn hàng #${orderId} đã gửi tới ${email} thành công! ID: ${info.messageId}`);
      return info;
    } catch (error) {
      this.logger.error(`Lỗi khi gửi email xác nhận đơn hàng #${orderId}:`, error);
      throw error;
    }
  }
}
